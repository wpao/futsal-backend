...
// delete user
app.delete("/users/delete/:id", accessValidator, async (req: Request, res: any) => {
  try {
    const { id } = req.params;
...
// delete booking
app.delete("/bookings/delete/:id", accessValidator, async (req: Request, res: any) => {
  try {
    const { id } = req.params;
...
app.post("/info", accessValidator, async (req: Request, res: any) => {
  try {
    ...
    if (!content || !idLapanganChange) {
      return res.status(400).json({ message: "Content and idLapanganChange are required" });
    }
